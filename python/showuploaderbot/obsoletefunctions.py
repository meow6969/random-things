...
def check_for_subtitle_changes_for_show(show_folder: str):
    last_subtitles_for_video: list[dict] | None = None
    last_subtitles_special_keys: dict[str, list] | None = None
    last_subtitles_special_keys_hash: int | None = None
    last_video_path: str | None = None
    last_video_name: str | None = None
    special_keys = ("tags/language", "index")
    video_subtitles_types: dict[int, dict] = {}
    current_episode_range: list[str] = []
    total_subtitle_changes = 0
    start_time = time.time_ns()
    time_spent_getting_subtitle_info = 0
    for season_folder in sorted(os.listdir(show_folder)):
        season_path = os.path.join(show_folder, season_folder)
        for episode_file_name in sorted(os.listdir(season_path)):
            episode_file_path = os.path.join(season_path, episode_file_name)
            s_time = time.time_ns()
            cur_episode_subtitles = get_all_subtitles_info_from_video(episode_file_path)
            time_spent_getting_subtitle_info += time.time_ns() - s_time
            if last_subtitles_for_video is None:  # this is only for very first ep
                last_subtitles_for_video = cur_episode_subtitles
                last_subtitles_special_keys = get_list_of_dict_keys_as_dict_list(
                    cur_episode_subtitles, *special_keys)
                last_video_path = episode_file_path
                last_video_name = episode_file_name
                last_subtitles_special_keys_hash = hash_dict_or_list(last_subtitles_special_keys)
                video_subtitles_types[last_subtitles_special_keys_hash] = {
                    "special-keys": last_subtitles_special_keys,
                    "episode-ranges": []
                }
            ...
            #     last_video_path = episode_file_path
            #     continue
            cur_episode_special_keys = get_list_of_dict_keys_as_dict_list(cur_episode_subtitles, *special_keys)
            if cur_episode_special_keys != last_subtitles_special_keys:
                cur_episode_special_keys_hash = hash_dict_or_list(cur_episode_special_keys)
                print(f"\n\n{CCs.FAIL}differing subtitles detected!\n"
                      f"last video ({last_video_path}) got\n"
                      f"{last_subtitles_special_keys_hash}: {json.dumps(last_subtitles_special_keys, indent=2)}\n"
                ...
                video_subtitles_types[last_subtitles_special_keys_hash]["episode-ranges"].append(current_episode_range)
                if cur_episode_special_keys_hash not in video_subtitles_types.keys():
                    video_subtitles_types[cur_episode_special_keys_hash] = {
                        "special-keys": cur_episode_special_keys,
                        "episode-ranges": []
                    }
                last_subtitles_for_video = cur_episode_subtitles
                last_subtitles_special_keys = cur_episode_special_keys
                last_subtitles_special_keys_hash = hash_dict_or_list(last_subtitles_special_keys)
                last_video_path = episode_file_path
    ...
    for subtitle_type in video_subtitles_types:
        print(f"{subtitle_type}: {json.dumps(video_subtitles_types[subtitle_type]['episode-ranges'])}")

    print(f"\n\nall subtitle types:\n"
          f"{json.dumps(video_subtitles_types, indent=2)}\n\n")
    print(f"{CCs.OKBLUE}"
          f"total subtitle changes:           {CCs.ENDC}", end="")
    if total_subtitle_changes == 0:
        print(f"{CCs.OKGREEN}{total_subtitle_changes}{CCs.ENDC} {CCs.OKCYAN}:3{CCs.ENDC}")
    elif total_subtitle_changes < 6:
        print(f"{CCs.UNDERLINE}{CCs.WARNING}{total_subtitle_changes}{CCs.ENDC}{CCs.OKCYAN} :({CCs.ENDC}")
    else:
        print(f"{CCs.UNDERLINE}{CCs.FAIL}{total_subtitle_changes}{CCs.ENDC}{CCs.OKCYAN} :c{CCs.ENDC}")
    total_time_spent = time.time_ns() - start_time
    print(f"{CCs.OKBLUE}" 
          f"total time spent:                 {int(total_time_spent / 1000000000)}s\n"
          f"time spent getting subtitle info: {time_spent_getting_subtitle_info / 1000000000}s\n"
          f"difference:                       {(total_time_spent - time_spent_getting_subtitle_info) / 1000000000}s"
          f"{CCs.ENDC}")
